import React from "react";
import styles from "../../styles/styles";

const Sponsored = () => {
  return (
    <div
      className={`${styles.section} hidden sm:block bg-white py-10 px-5 mb-12 cursor-pointer rounded-xl`}
    >
      <div className="flex justify-between w-full">
        <div className="flex items-start">
          <img
            src="https://logos-world.net/wp-content/uploads/2020/04/Sony-Logo.png"
            alt=""
            style={{ width: "150px", objectFit: "contain" }}
          />
        </div>
        <div className="flex items-start">
          <img
            src="https://logos-world.net/wp-content/uploads/2020/08/Dell-Logo-1989-2016.png"
            style={{ width: "150px", objectFit: "contain" }}
            alt=""
          />
        </div>
        <div className="flex items-start">
          <img
            src="https://raw.githubusercontent.com/Rahuljagwan/images/refs/heads/main/png-transparent-samsung-logo-samsung-galaxy-s9-samsung-electronics-logo-samsung-kies-samsung-blue-text-trademark-removebg-preview%20(1).png"
            style={{ width: "170px", objectFit: "contain", marginTop: "1px" }}
            alt=""
          />
        </div>
        <div className="flex items-start">
          <img
            src="https://raw.githubusercontent.com/Rahuljagwan/images/refs/heads/main/png-clipart-carplay-macbook-pro-apple-imac-united-states-name-text-logo-removebg-preview.png"
            style={{ width: "150px", objectFit: "contain", marginTop: "10px" }}
            alt=""
          />
        </div>
        <div className="flex items-start">
          <img
            src="data:image/png;base64,"
            style={{ width: "150px", objectFit: "contain" }}
            alt=""
          />
        </div>
      </div>
    </div>
  );
};

export default Sponsored;
